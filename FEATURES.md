...
- Local Group Demo badges show newly formed groups and unread incoming group messages; opening Group Demo clears those local-only indicators.
...
- Debug-only bounded direct-traffic producer exercises bidirectional `RELIABLE_ACKED` messaging at a configured rate and UTF-8 text size, with sent/acked/failed/pending counters and no durable outbox.
...
- The host runner now covers accepted and rejected pairing, accepted and rejected Group Demo invitations, duplicate Group Demo invite handling, friendship removal notification, malformed application input and stale response rejection, direct/group coexistence and ownership release, removed-friend authorization and re-friending, idempotent repeated friendship requests, offline-send failure without durable queuing, persisted friendship versus ephemeral Group Demo state across process restart, three-device coordinator failure/migration, bidirectional burst delivery, bidirectional fixed-rate delivery, normative 32-byte/4096-byte payload delivery, duplicate Connect/link handling, and Android Bluetooth interruption/recovery in addition to pairing, bidirectional direct chat, group coordinator convergence, and bidirectional group chat. Endpoint-rotation scenarios remain to be added.
